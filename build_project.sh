...
# Build all HLS IP
cd ./HLS/build
./create_HLS_projects.py --clean >> $MAIN_DIR/build.log
./create_HLS_projects.py --export_ip >> $MAIN_DIR/build.log

TIME_AFTER_HLS=$(date +%s)
echo "INFO [build_project.sh] HLS build time: $(($TIME_AFTER_HLS - UNIX_TIME_START)) seconds" >> $MAIN_DIR/build.log
echo "INFO [build_project.sh] HLS build time: $(($TIME_AFTER_HLS - UNIX_TIME_START)) seconds" 
...
# Calculate Vitis HLS build time
HLS_ELAPSED_TIME=$((TIME_AFTER_HLS - UNIX_TIME_START))
HLS_HOURS=$((HLS_ELAPSED_TIME / 3600))
HLS_MINUTES=$(((HLS_ELAPSED_TIME % 3600) / 60))
HLS_SECONDS=$((HLS_ELAPSED_TIME % 60))
...
PETALINUX_BUILD_ELAPSED_TIME=$((END_TIME - TIME_AFTER_HLS))
...
echo "INFO [build_project.sh] Vitis HLS build time: ${HLS_HOURS}h ${HLS_MINUTES}m ${HLS_SECONDS}s" >> $MAIN_DIR/build.log
echo "INFO [build_project.sh] Vitis HLS build time: ${HLS_HOURS}h ${HLS_MINUTES}m ${HLS_SECONDS}s"
