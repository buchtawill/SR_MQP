...
def make_hls_deconv_func(name:str, in_ch:int, out_ch:int, kernel_size:int, in_width_pix:int, num_pe:int):
    ...
    func =  f"void conv_{name}(ch_stream_t tile_in[IN_CHN_LAYER_{name.upper()}], upscaled_stream_t map_out[OUT_CHN_LAYER_{name.upper()}]){{\n"
    func += f"    // NOTE: This function was auto generated. Do not edit here, edit FSRCNN/conv_ideal.py\n"
    ...
    for i in range(kernel_size-1):
        func += f"    hls::stream<fixed_4_8_t, INPUT_WIDTH_PIX*2> psum{i+1}[NUM_PE_LAYER_{name.upper()}];\n"
    ...
    # Get the next value from get_next_tconv. If it's not a pad value and it's not (num_pe_loops - 1), write to inbuf
    ...
    # Slider is done, go across the row
    func += f"            // Go across the row\n"
    ...
    func += f"                        map_out[filter].write(pre_activation);\n"
    func += f"                    }}\n"
