...
int bilinear_interpolation_calculations(pixel_t image_in[HEIGHT_IN * WIDTH_IN * CHANNELS],
    ...
    fixed widthRatio  = static_cast<fixed>(WIDTH_IN - 1) / static_cast<fixed>(WIDTH_OUT - 1);
    fixed heightRatio = static_cast<fixed>(HEIGHT_IN - 1) / static_cast<fixed>(HEIGHT_OUT - 1);
    ...
    for (int y_out = 0; y_out < HEIGHT_OUT; ++y_out) {
        ...
        for (int x_out = 0; x_out < WIDTH_OUT; ++x_out) {
			...
			#pragma HLS UNROLL factor=2
                ...
                image_out[out_index] = static_cast<pixel_t>(fixed(interpolated_value) + fixed(0.5));
            }
        }
    }
	...
	while(i < NUM_TRANSFERS){
		...
		while(!in_stream.empty()){
			...
			for(int transfer_pixel = 0; transfer_pixel < PIXELS_PER_TRANSFER; transfer_pixel++){
				...
				unloaded[i * PIXELS_PER_TRANSFER + transfer_pixel] = temp_pixel;
			}
